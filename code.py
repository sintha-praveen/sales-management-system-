...
# -----------------------------
        ...
        detail_query += " ORDER BY cs.date DESC"
    ...
    # ==================================        
        ...
        if question == questions[3]:
            query = "SELECT * FROM customer_sales WHERE status = 'Open'" 
        ...
        if question == questions[5]:
            query = "SELECT SUM(received_amount) AS Total_received_amount FROM customer_sales"

            cur.execute(query)
            

        if question == questions[6]:
            query = "SELECT SUM(pending_amount) AS Total_pending_amount FROM customer_sales"

            cur.execute(query)

        if question == questions[7]:
            query = """SELECT cs.branch_id,b.branch_name, COUNT(*) AS total_sales
                    FROM customer_sales cs
                    JOIN branches b
                    ON cs.branch_id = b.branch_id
        ...
        if question == questions[9]:
            query = """
                SELECT cs.*, COALESCE(SUM(ps.amount_paid), 0) AS total_received
                FROM customer_sales cs
        ...
        if question == questions[13]:
            query = "SELECT * FROM customer_sales ORDER BY gross_sales DESC LIMIT 3"
        ...
        if st.button('Execute Query'):
            ...
            if len(df.columns) == 1:
                st.dataframe(df, hide_index=True, height=70, width=300)
            else:
